...
| T-071 Improve Notes markdown formatting prompts | Backlog | P2, Notes quality. Improve live and final Notes prompts so output consistently uses useful markdown hierarchy, including H1/H2/H3 where appropriate, bullets, sub-bullets, concise sections, and cleaner live update formatting. Preserve append-only live patch architecture and final polish/dedupe behaviour. |
| T-072 Review and tune Notes/Formify GPT prompts | Backlog | P2, backend prompt/performance quality pass. Review all active GPT prompts in the current app context, including transcription revision, Forms extraction/final parse, Notes live patch updates, Notes finalisation, and Notes transform prompts for Summarise/Reorganise. Improve markdown hierarchy, bullets/sub-bullets, section quality, answered-vs-unresolved question handling, relevant example preservation, live update formatting, and token efficiency where safe. Remove unwanted Quick Checklist behaviour unless explicitly requested. Preserve distinct Summarise/Reorganise product purposes. Do not change WebSocket contracts, frontend behaviour, batching/reconnect/cap logic, or pricing/paywall language. |
