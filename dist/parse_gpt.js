...
const tokenCounter4o = (0, tiktoken_1.encoding_for_model)("gpt-4o-mini");
const tokenCounter = (0, tiktoken_1.encoding_for_model)("gpt-4.1");
const REVISE_SYS_TXT = `You are a transcription editor working in a professional, Australian context where meetings often involve topics 
        ...
        `;
const EXTRACT_SYS_TXT = `You are an attribute extraction agent specialized for an Australian environment.
        ...
        `;
const FINAL_SYS_TXT = `You are an attribute extraction revision assistant designed to verify and correct structured data extracted from spoken text.
...
async function extractAttributesFromText(correctedText, template, currAttributes) {
    ...
    try {
        const parsed = JSON.parse(content);
        const cleaned = Object.fromEntries(Object.entries(parsed.parsedAttributes).filter(([, v]) => v !== "" && v !== "N/A"));
        return cleaned;
    }
    catch {
